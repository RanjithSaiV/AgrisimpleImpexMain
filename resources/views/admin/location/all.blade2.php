        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-muted">City Name</li>
                    <!--end::Item-->
                </ul>
                <!--end::Breadcrumb-->
            </div>
            ...
            <div class="card card-flush">
                ...
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                    ...
                    <div class="card-toolbar flex-row-fluid justify-content-end gap-5 ml-2">
                        <!--begin::Add product-->
                        <a href="{{ route('location.create') }}" class="btn btn-primary">Add City</a>
                        <!--end::Add product-->
                    </div>
                    <!--end::Card toolbar-->
                </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                        ...
                        <tbody class="fw-semibold text-gray-600">
                            ...
                            @foreach ($data as $item)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{ $item->name_of_donor }}</td>
                                    <td> <img src="{{ asset($item->donor_image) }}" alt="{{ $item->name_of_donor }}"
                                            width="50"/></td>
                                    <td>{{ $item->designation }}</td>
                                    <td>{{ $item->nature_of_donation }}</td>
                                    <td>{{ $item->amount }}</td>
                                    <td>{{ $item->status }}</td>
                                    <td><a href="{{ route('donor.edit', $item->id) }}">Edit</a></td>
                                    <td>
                                        <form action="{{ route('donor.destroy', $item->id) }}" method="POST">
                                            @csrf
                                            @method('delete')
