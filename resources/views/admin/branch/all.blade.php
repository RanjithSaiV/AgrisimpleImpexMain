        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-muted">Branch Name</li>
                    <!--end::Item-->
                </ul>
                <!--end::Breadcrumb-->
            </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                        <thead>
                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-100px">Petrol price</th>
                                <th class="min-w-100px">Diesel Price</th>
                                <th class="min-w-100px">Landmark </th>
                                <th class="min-w-100px">City Name</th>
                                <th class="min-w-100px">State</th>
                                <th class="min-w-100px">Add Fuel</th>
                        ...
                        <tbody class="fw-semibold text-gray-600">
                            <!-- Donor list data will be inserted here -->
                            @php $i=1; @endphp
                            @foreach ($data as $item)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{ $item->name }}</td>
                                    <td>{{ $item->petrol_price }}</td>
                                    <td>{{ $item->diesel_price }}</td>
                                    <td>{{ $item->landmark }}</td>
                                    <td>{{ $item->location->city ?? '' }}</td>
                                    <td>{{ $item->location->state?? '' }}</td>
                                    <td><a class="btn btn-primary" class="text-white" href="{{ route('fuels.index', ['id' => $item->id]) }}">Fuel</a></td>
                                    <td>{{ $item->status }}</td>
                                    <td><a class="btn btn-warning" href="{{ route('branch.edit', $item->id) }}">Edit</a></td>
